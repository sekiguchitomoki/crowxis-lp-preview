...
(function(){
  ...
  if(qaFinal){
    ...
    // v4: 動画が途中で固まる端末でもオープニングで固着しないハードフォールバック
    setTimeout(openSite, 8000);
  ...
  // v4: モバイル/タッチ環境では発熱・重さ軽減のためパーティクルを無効化
  var enableParticles = window.matchMedia('(min-width:900px) and (pointer:fine)').matches;
  if(!enableParticles){ cv.style.display = 'none'; }
  else{
